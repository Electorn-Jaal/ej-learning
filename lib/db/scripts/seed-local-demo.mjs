...
// One password for every demo account, because they are demo accounts: the
// script refuses any database but ej_learning_local or ej_learning_test, so
// these credentials never reach anything but a throwaway local copy, and a
// random string per account only made them tedious to type while testing.
...
export async function seedLocalDemo(client, database) {
  ...
  for (const spec of [
    ...
    if (spec.role==='TEACHER') {
      ...
      // The class studies these whether or not this teacher stays. Student
      // screens read the curriculum, not the staffing, so the fixture has to
      // state both - a class with teachers but no class_subjects rows is the
      // exact shape that used to render an empty page.
      await client.query(`INSERT INTO core.class_subjects (class_id,subject_id,origin)
        VALUES ($1,$2,'ROSTER') ON CONFLICT (class_id,subject_id) DO NOTHING`,[spec.classId,subject.id]);
      if (spec.alsoTeaches) {
        await client.query(`INSERT INTO core.class_subjects (class_id,subject_id,origin)
          VALUES ($1,$2,'ROSTER') ON CONFLICT (class_id,subject_id) DO NOTHING`,[spec.classId,spec.alsoTeaches]);
      }
    }
  ...
  // A second section, so "move the class on" has somewhere to move to and the
  // ordering of the picker is actually exercised.
  await client.query(`INSERT INTO content.source_outline_nodes (source_material_id,outline_code,node_type,title,page_from,page_to,sequence_no,planning_period_no,status)
    VALUES ($1,'MOCK-LOCAL-OUTLINE-2','SECTION','Туршилтын хоёр дахь хэсэг',2,3,2,1,'APPROVED')`,[material.id]);
  // The maths classes work from this book. Attaching it here rather than in
  // the class_subjects insert above because the book does not exist yet at
  // that point.
  await client.query(`UPDATE core.class_subjects SET source_material_id=$1
    WHERE subject_id=$2 AND class_id = ANY($3::bigint[])`,[material.id,subject.id,[klass.id,klassB.id]]);
  // A second book, for a different subject, that no maths class studies. It
  // exists so the "this section is not from your book" refusal has something
  // real to refuse - a foreign key alone would happily accept it.
  const materialB = await one(`INSERT INTO content.source_materials (source_code,subject_id,title,material_type,total_pages,status,data_quality_status,notes)
    VALUES ('MOCK-LOCAL-BOOK-B',$1,'Local demo — өөр хичээлийн ном','TEXTBOOK',1,'APPROVED','COMPLETE','Synthetic setup fixture') RETURNING id`,[subject2.id]);
  await client.query(`INSERT INTO content.source_outline_nodes (source_material_id,outline_code,node_type,title,page_from,page_to,sequence_no,status)
    VALUES ($1,'MOCK-LOCAL-OUTLINE-B','SECTION','Өөр номын хэсэг',1,1,1,'APPROVED')`,[materialB.id]);
  await client.query(`UPDATE core.class_subjects SET source_material_id=$1
    WHERE subject_id=$2 AND class_id=$3`,[materialB.id,subject2.id,klass.id]);
