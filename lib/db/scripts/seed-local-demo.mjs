...
export async function seedLocalDemo(client, database) {
  ...
  const klass = await one(`INSERT INTO core.classes (class_code,grade_level_id,name_mn,school_year,data_origin)
    VALUES ('MOCK-LOCAL-9A',$1,'Туршилтын 9А',$2,'MOCK') RETURNING id`, [grade.id,schoolYear]);
  const student = await one("INSERT INTO core.students (student_code,display_name,data_origin) VALUES ('MOCK-LOCAL-STUDENT','Туршилтын сурагч','MOCK') RETURNING id");
  await client.query('INSERT INTO core.student_enrollments (student_id,class_id) VALUES ($1,$2)',[student.id,klass.id]);
  const accounts = [];
  let adminId;
  for (const [username, role, displayName] of [
    ['demo-admin','ADMIN','Туршилтын админ'],
    ['demo-teacher','TEACHER','Туршилтын багш'],
    ['demo-student','STUDENT','Туршилтын сурагч'],
  ]) {
    const password = randomBytes(18).toString('base64url');
    const user = await one(`INSERT INTO core.users (username,password_hash,display_name,student_id)
      VALUES ($1,$2,$3,$4) RETURNING id`,[username,await hashPassword(password),displayName,role==='STUDENT'?student.id:null]);
    await client.query('INSERT INTO core.user_roles (user_id,role) VALUES ($1,$2)',[user.id,role]);
    ...
    if (role==='TEACHER') {
      const teacher = await one(`INSERT INTO core.teachers (user_id,teacher_code,subject_id,data_origin)
        VALUES ($1,'MOCK-LOCAL-TEACHER',$2,'MOCK') RETURNING id`,[user.id,subject.id]);
      await client.query('INSERT INTO core.class_teachers (class_id,teacher_id,subject_id) VALUES ($1,$2,$3)',[klass.id,teacher.id,subject.id]);
    }
